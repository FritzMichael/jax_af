...
def get_jax_BP_fwd(constants: SarConstants = SarConstants()):
    ...
    jax_BP = jax_kernel(
        forward_kernel, num_outputs=1, enable_backward=False, has_side_effect=False
    )
    return jax_BP


def get_jax_BP_auto_backward(constants: SarConstants = SarConstants()):
    forward_kernel = get_forward_kernel(constants)
    jax_BP = jax_kernel(
        forward_kernel, num_outputs=1, enable_backward=True, has_side_effect=False
    )
    return jax_BP
...
def get_jax_BP(constants: SarConstants = SarConstants()):
    ...
    def _sar_BP_bwd(num_radar_pos, left_side_only, res, adj_output_image):
        (pixel_pos, range_profiles, radar_pos, radar_vel) = res
...
def run_jax_BP(
    ...
    range_profiles_jax = jnp.array(
        convert_rp_to_vec2(range_profiles), dtype=jnp.float32
    )
    ...
    output_image = output_image.view(jnp.complex64).reshape(
        output_image.shape[0], output_image.shape[1]
    )

    # calc image, entropy and grad for testing
    jitted_fun = (
        jax.value_and_grad(partial(_run_jax_BP, loss_fn=loss_fn), argnums=2)  # ,
        # static_argnums=(4, 5),
    )
    args_jax = (
        pixel_pos_jax,
    ...
    entropy, grad = jitted_fun(*args_jax)

    return output_image, entropy, grad, (jitted_fun, args_jax)


def run_jax_BP_auto_backward(
    pixel_pos: np.ndarray,
    range_profiles: np.ndarray,
    radar_pos: np.ndarray,
    radar_vel: np.ndarray,
    ...
    jax_BP = get_jax_BP_auto_backward(constants)
    num_radar_pos = int(radar_pos.shape[0])
    left_side_only = False
    ...
    jitted_fun = jax.value_and_grad(partial(_run_jax_BP, loss_fn=loss_fn), argnums=2)
    args_jax = (
        pixel_pos_jax,
        range_profiles_jax,
        radar_pos_jax,
